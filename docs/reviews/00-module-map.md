...
## Autonomous gap pass — round 29 (copy portal link)
...
## Couples & Events platform — Per-space layout design & approval
✅ The couple marks each selected space as draft/designed/submitted with notes; submit
...
## Couples & Events platform — RSVP summary dashboards
✅ The couple's Guests tab shows an RSVP summary (invited / attending / not attending /
...
## Couples & Events platform — Wayfinding From-any-point
...
appear for venue pre-login; lodging-event guests see the Lodging tab; couple
itinerary progress step; manual RSVP includes assigned events. See
docs/reviews/95-venue-operations-rollup.md.
...
and RSVPs per event; couple sees RSVPs per event. See
docs/reviews/93-guest-events-itinerary.md.
...
to each couple with setup auto-suggest; couple Package tab views their package and
adds/removes paid add-ons. See docs/reviews/91-wedding-packages-addons.md.

## Couples & Events platform — Couple checklist, vendors & venue setup (round 12)
✅ Couple's own prep checklist tab; couple vendors (pick venue preferred / add
custom + status); venue per-couple setup & staffing panel (space/day/who/when).
See docs/reviews/90-couple-checklist-vendors-setup.md.
...
## Couples & Events platform — Continued gap pass (round 9)
✅ Couple headcount + space-capacity warnings; venue per-meal catering counts;
venue couple search; no-spaces guidance in couple create/edit.
See docs/reviews/87-continued-gap-pass-9.md.

## Couples & Events platform — Continued gap pass (round 8)
✅ Layout approval status/resubmit guidance; schedule + RSVP day dates shown;
completed event read-only; venue RSVP summary ignores stale RSVPs; couple portal
polls for new guest RSVPs. See docs/reviews/86-continued-gap-pass-8.md.

## Couples & Events platform — Continued gap pass (round 7)
✅ Chat timestamps; no duplicate 'Entrance' in wayfinding; guest CSV export
includes RSVP details; couple Overview shows weather forecast.
See docs/reviews/85-continued-gap-pass-7.md.
...
progress fix; venue rules on couple Design tab; push shared settings to couples
(merge-preserving customizations). See docs/reviews/82-continued-gap-pass-4.md.

## Couples & Events platform — Continued gap pass (round 3)
✅ Non-negative couple RSVP summary; contact-venue email; guest RSVP reminders +
no-response CSV; weather auto-fetch feedback; wayfinding availability fix;
guest portal preview mode; RSVP special-needs/notes visible to couple + venue.
See docs/reviews/81-continued-gap-pass-3.md.

## Couples & Events platform — Continued gap pass (round 2)
✅ Fixed multi-day celebration countdown (was "passed" mid-event); added pure
timezone-safe `celebrationStatusDays` helper + unit tests. See
docs/reviews/79-continued-gap-pass.md.
