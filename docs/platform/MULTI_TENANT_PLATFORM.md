...
supabase/migrations/0014_geoapify_address_quality.sql
```
...
`https://app/i/<token>` (no `?` or `#` — Outlook/Brevo wrappers break those).
...
The yellow **Node.js 20 is deprecated** line is a warning, not the failure.
