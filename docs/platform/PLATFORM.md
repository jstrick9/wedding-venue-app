...
|---|---|---|
| Platform/venue account bootstrap | ✅ Platform role, venue-admin onboarding, and platform console management exist; first platform owner still requires one-time SQL bootstrap and live RLS verification. | `platformAdminService`, `PlatformAdminPortal`, `VenueAdminOnboarding`, migrations `0006`–`0008` |
...
7. **`0008_platform_console_management.sql`** — immutable generated slugs, tenant suspension/reactivation, invite reissue/revocation, and global/per-venue operational metrics.

Apply all eight (`supabase db push`, or paste each into the SQL editor) only in a non-production test project first. Then bootstrap the first `platform_owner` using the SQL in `docs/platform/MULTI_TENANT_PLATFORM.md`. Setting `VITE_BACKEND_PROVIDER=supabase` turns on the available seams, but it does not make every local couple/guest/operations workflow cloud-backed. Complete the platform-owner → managed-admin → tenant-isolation → couple/guest browser smoke test before migrating real venue data.
