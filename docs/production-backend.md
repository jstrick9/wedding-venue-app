...
3. Configure either `BREVO_API_KEY` or `RESEND_API_KEY` as an Edge Function secret. `PASSWORD_RESET_APP_URL` is required in production and must be the branded application origin. `PASSWORD_RESET_FROM_EMAIL` is also required and must be a verified branded sender.
