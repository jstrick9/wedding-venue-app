...
3. Configure either `BREVO_API_KEY` or `RESEND_API_KEY` as an Edge Function secret. The production deployment workflow manages and synchronizes the non-confidential `PUBLIC_APP_URL` and verified `PASSWORD_RESET_FROM_EMAIL` settings before deployment. There is one canonical application origin and one sender for the entire SaaS environment—never per-tenant URL settings. `PASSWORD_RESET_APP_URL` remains accepted only as a legacy fallback.
