...
**Phase 2 progress: original 46/46 — COMPLETE (#261). Migrations 0018–0020 are applied and live-verified (#271); migration 0021 is applied and its 11 account-binding/internal-wrapper functions are audited and locally behavior-verified (#273). Migration 0022 is applied in production; its two service-only recovery functions are audited and behavior-verified in PGlite, and the resulting branded live delivery path is proven (#274). Current finite inventory: 59/59 examined.**
...
| 2.58 | begin_password_reset_request * | done (production applied; local behavior proven) | #274: service-only, hash-only audit rows; global → requester → email advisory-lock order; atomic rolling-window throttles; anon/authenticated execute revoked; PGlite ACL/throttle behavior passed; live branded request/delivery path proven |
| 2.59 | get_password_reset_account_context * | done (production applied; local behavior proven) | #274: service-only canonical account lookup; active surface membership required; venue status enforced; minimal server-derived delivery context; anon/authenticated execute revoked; PGlite eligibility behavior passed; live eligible venue-admin delivery proven |

## C. Phase 3 — authorization proof matrix (31 tables × 5 role classes after migration 0022)

**Method (#262/#273/#274):** the original 29-table deployed-schema anon column was live-proven with the publishable key; guest/couple data uses security-definer RPCs. Migration 0021 is applied and adds `portal_accounts` as table 30; its policy/ACL behavior is PGlite-proven (#273), with account-journey live proof pending. Migration 0022 is applied and adds deny-by-default `password_reset_requests` as table 31; its RLS/ACL behavior is PGlite-proven and its live branded request/delivery path is proven (#274). Venue/platform cells otherwise remain policy-derived except the #272 live venue-claim coverage. Verdicts: `certified` (live evidence) / `policy` (derived from inventory, live proof pending) / `n-a`.
...
| password_reset_requests *(0022 applied; ACL behavior locally proven)* | svc: RLS enabled, no policies; table + RPC execute revoked | svc | svc | svc | svc |
...
| 4.12 | Auth/session lifecycle: sign-in, restore, sign-out, role routing | AuthBackend, session persistence | **complete live (#275)** | #274: strict active surface entitlement on restore/sign-in; target-surface persistence cleared before network teardown; platform/venue sessions isolated; direct reset proof captured before bootstrap and URL-scrubbed; token-hash/PKCE/legacy proofs bounded. #275 fixed the live-proven one-time-proof reuse failure after a rejected password candidate with a dedicated memory-only client, same-proof/user/surface binding, serialized submits, policy/history/network retry retention, navigation abandonment, PKCE transfer, and success-only global revocation. Full gates, exact-SHA CI/deployment, both canonical reset routes, and fresh-link current-password reject → same-screen different-password save → branded sign-in passed; prior password rejected. |
...
| 5.5 | request-password-reset | **complete live (#274/#275)** | #274: public neutral-response door; service-derived eligibility; hash-only atomic throttling; server-owned branded origin/sender; background delivery with provider deadlines/failover; fragment proof; explicit no-JWT deployment step. The Function uses one project-wide origin for all tenants with environment overrides and a legacy URL fallback; public production defaults are versioned server-side without broadening the least-privilege deployment token. #275: fresh branded venue-admin email → actionable current-password rejection → same-screen different-password save → branded sign-in passed in production; prior password rejected. |
...
| 8.2 | Claim → configure → couple portal publish → guest RSVP → venue sees submission | **in-progress; live pending** | Personal-account implementation deployed/local contract complete #273. #276 corrected claim/guest verification assumptions and added failure-safe mutation accounting; remediation SHA `7152bc5` is CI-green/deployed, and fresh throwaway artifacts are still required. |
| 8.3 | Reissue invite → stable personal account → old token denied/current session evaluated | **in-progress; live pending** | Stable account/reissue backend contract #273. Released #276 F-276-4 deterministically re-gates an already-open old-token portal after authoritative `not_found`; fresh couple invite needed. |
| 8.4 | Suspend venue → each console's behavior | **in-progress; live pending** | Released #276 F-276-4 propagates `venue_unavailable` and re-gates already-open couple/guest portals while preserving transient retry behavior; authorized throwaway suspension/reactivation and live console proof pending. |
| 8.5 | Concurrent RSVP race (two guests, one couple) | **harness complete; live pending** | Released #276 replaced legacy anonymous #248 harness with two distinct personal accounts/JWTs, bidirectional cross-account denial, safe oversized negative writes, projected-RSVP readback, and an always-emitted redacted mutation ledger; 6 harness tests pass. |
| 8.6 | Guest token expiry / access window edges | **in-progress; live pending** | Released #276 F-276-4 preserves `expired` as a terminal denial and re-gates an already-open guest portal on its next poll; live before/edge/after proof requires an operator-controlled throwaway expiry boundary. |
...
| 1 fresh email-backed couple/collaborator invitation to a throwaway email + known strong password | personal-account claim/reissue live proof (#273 / journeys 8.2–8.3) | **requested for #276 live continuation** — migration 0021, claim Functions, and corrected frontend SHA `7152bc5` are deployed/green |
