...
| 4.12 | Auth/session lifecycle: sign-in, restore, sign-out, role routing | AuthBackend, session persistence | **retry correction complete locally; deployment + live reject→retry proof pending** | #274: strict active surface entitlement on restore/sign-in; target-surface persistence cleared before network teardown; platform/venue sessions isolated; direct reset proof captured before bootstrap and URL-scrubbed; token-hash/PKCE/legacy proofs bounded. #275: live testing exposed one-time-proof reuse failure after a rejected password candidate. The correction keeps the exchanged capability in a dedicated memory-only client, binds retries to the same proof/user/surface, serializes submits, retains password-policy/history/network rejections, abandons on navigation, preserves PKCE transfer, and globally revokes only after success. Current-password and history rejection copy is white-label; focused and full gates passed; deployment and fresh-link live retest pending. |
...
| 5.5 | request-password-reset | deployed; production configuration + branded inbox delivery proven | #274: public neutral-response door; service-derived eligibility; hash-only atomic throttling; server-owned branded origin/sender; background delivery with provider deadlines/failover; fragment proof; explicit no-JWT deployment step. Live follow-up found the production origin unset and the password-specific URL name duplicated existing project configuration; the function now uses one project-wide origin for all tenants with environment overrides and the old name as a compatibility fallback. A workflow sync attempt correctly failed because the least-privilege deploy token cannot administer secrets; the public production origin/sender are therefore versioned as server-side deployment defaults instead of broadening token privileges. Live venue-admin request and branded inbox delivery passed; frontend reject→retry correction still requires deployment and one fresh-link retest. |
