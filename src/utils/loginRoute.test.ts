...
describe('loginHashAfterLogout', () => {
  ...
  it('does not rewrite public portal or onboarding hashes', () => {
    ...
    expect(loginHashAfterLogout('', 'seven-paths-manor', '/accept-invite/abcdef0123456789')).toBe('');
    expect(loginHashAfterLogout('', 'seven-paths-manor', '/reset/platform')).toBe('');
    expect(loginHashAfterLogout('#/password-reset')).toBe('#/password-reset');
  });
});
