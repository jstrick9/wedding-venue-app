...
export default function PlatformVenueChatPanel({ organizationId, organizationName = 'venue', senderSide = 'venue', inline = false }: PlatformVenueChatPanelProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setError('');
      setMessages(await withTimeout(
        listPlatformVenueMessages(effectiveOrganizationId, senderSide),
        20000,
        'Loading chat timed out. Try again, or sign in again at Platform login if this keeps happening.',
      ));
  ...
  const send = async () => {
    ...
    try {
      const message = await withTimeout(
        sendPlatformVenueMessage(effectiveOrganizationId, draft, senderSide),
        20000,
        'Sending the message timed out. Try again, or sign in again at Platform login if this keeps happening.',
      );
      setMessages((current) => current.some((item) => item.id === message.id) ? current : [...current, message]);
      setDraft('');
    } catch (err) {
  ...
  return (
    <div className={`${inline ? '' : 'rounded-2xl border border-gray-200 bg-white shadow-xl'} flex min-h-[420px] flex-col overflow-hidden`}>
      ...
      <div className="flex gap-2 border-t border-gray-200 bg-white p-3"><input value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void send(); }} placeholder={`Message ${organizationName}…`} className="flex-1 rounded-lg border border-gray-300 px-3 py-2 text-sm" aria-label={`Platform chat with ${organizationName}`} /><button type="button" onClick={() => void send()} disabled={!draft.trim() || !user || sending} className="rounded-lg bg-indigo-700 px-4 py-2 text-xs font-bold text-white disabled:opacity-50">{sending ? 'Sending…' : 'Send'}</button></div>
    </div>
  );
}
