...
export function PropertiesPanel({
  ...
  // Get effective capacity (custom or default).
  // For seating-only types, capacity follows chair count.
  ...
  const tableGuests = table 
    ? guests.filter(g => table.guests.includes(g.id))
    : [];
  ...
  // Get linen color info
  const getLinenColorInfo = (colorId?: string) => {
    const defaultColor = { id: 'white', name: 'White', hex: '#FFFFFF', textColor: '#374151', enabled: true };
    return linenColors.find(c => c.id === colorId) || linenColors[0] || defaultColor;
  };

  // Collapsed state - just show toggle button with expand arrow when item is selected
  if (!visible) {
    return (
      <div
      ...
      >
        ...
        {selectedId && (
          ...
          >
            ...
            <span className="text-xs whitespace-nowrap rotate-90 origin-center mt-4 opacity-70 group-hover:opacity-100">Open</span>
          </button>
        )}
      </div>
    );
  ...
  return (
    <div className="w-80 bg-gray-50 border-l border-gray-200 flex flex-col shadow-sm">
      ...
      <div
        ...
        style={{ background: `linear-gradient(to right, ${config.primaryColor}, ${config.primaryDark})`, color: config.headerTextColor }}
      >
      ...
      <div className="flex-1 overflow-auto p-4 bg-gray-100">
        {!item ? (
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
              <div className="flex items-center gap-3 mb-3">
                <div 
                  ...
                  style={{ 
                    backgroundColor: table && table.hasLinen !== false 
                ...
                >
                  <span style={{ 
                    color: table && table.hasLinen !== false 
              ...
              {spec?.imageUrl && (
                <button
                  ...
                  style={{ color: config.primaryColor, borderColor: `${config.primaryColor}33`, backgroundColor: `${config.primaryColor}0D` }}
                >
                  📷 View Actual Image
                </button>
              )}
              ...
              {Array.isArray((spec as any)?.images) && (spec as any).images.length > 0 && (
                <div className="mt-3">
                  <div className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Image Gallery</div>
                  <div className="grid grid-cols-3 gap-2">
                    {(spec as any).images.slice(0, 6).map((img: any) => (
                      <button
                        key={img.id}
                        type="button"
                        onClick={() => onViewImage(img.url, img.label || spec?.name || 'Fixture Image')}
                      ...
                      >
                        <img src={img.url} alt={img.label || spec?.name} className="w-full h-16 object-cover" />
                        {img.label && (
            ...
            <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
              <label className="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Applied Design</label>
              <select
            ...
            {/* Fixture Variant Selection (e.g., Greenery/Drapery Wall) */}
              ...
              <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {fixtureSpec.variants.map((variant) => (
                    <button
                      key={variant.id}
                      onClick={() => onUpdateFixture(fixture.id, { variant: variant.id, variantColor: variant.color })}
                ...
                {fixture.variant && (
                  <div className="mt-3 pt-3 border-t border-gray-200">
                    <div className="text-xs text-gray-500 mb-2">Current: <strong>{fixtureSpec.variants.find(v => v.id === fixture.variant)?.name || 'Default'}</strong></div>
                    <button
                      onClick={() => onUpdateFixture(fixture.id, { variant: undefined, variantColor: undefined })}
              ...
              <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className="h-1.5 flex-1 bg-gray-200 rounded-full overflow-hidden" style={{ width: '80px' }}>
                        <div 
                          className="h-full bg-[#4A1942] rounded-full transition-all"
                          style={{ width: `${(effectiveCapacity / tableSpec.capacity) * 100}%` }}
                        />
                      </div>
                      <span className="text-xs text-gray-500">{effectiveCapacity}/{tableSpec.capacity}</span>
                    </div>
                    {table.customCapacity !== undefined && table.customCapacity !== tableSpec.capacity && (
                      <button
            ...
            {table && tableSpec && !tableSpec.isSeatingType && (
              <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                ...
                <div className="flex items-center gap-3 mb-4">
                  <button
                    onClick={() => onUpdateTable(table.id, { hasLinen: table.hasLinen === false ? true : false })}
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-5 gap-2">
                      {linenColors.map(color => (
                        <button
                          key={color.id}
                          onClick={() => onUpdateTable(table.id, { linenColor: color.id as any })}
            ...
            {table && tableSpec && (() => {
              ...
              return (
                <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                  ...
                  {/* Show Chairs Toggle (hidden for seating-only types) */}
                  {!isSeatingType && (
                    <div className="flex items-center gap-3 mb-4">
                      <button
                        onClick={() => onUpdateTable(table.id, { showChairs: !showChairs })}
                        className={`relative w-12 h-6 rounded-full transition-colors ${
                    ...
                    <>
                      ...
                      <div className="mb-4">
                        ...
                        <select
                          ...
                          onChange={(e) => onUpdateTable(table.id, {
                        ...
                        <div className="mt-2 flex items-center gap-2">
                          <div 
                            ...
                            style={{ 
                              backgroundColor: chairSpecs.find(c => c.id === currentChairType)?.color || '#FFFFFF',
                              borderColor: '#666666'
                            }}
                          />
                      ...
                      <div>
                        ...
                        <div className="flex flex-col gap-2">
                          <div className="flex items-center gap-3">
                            ...
                            <button
                              onClick={() => {
                                const newValue = Math.min(tableSpec.capacity, chairCount + 1);
                                  onUpdateTable(table.id, {
                                    chairCount: newValue,
                                    ...(isSeatingType ? { showChairs: true } : {}),
                                  });
                          ...
                          {/* Chair count indicator */}
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <div className="h-1.5 flex-1 bg-gray-200 rounded-full overflow-hidden" style={{ width: '80px' }}>
                                <div 
                                  className="h-full bg-[#4A1942] rounded-full transition-all"
                                  style={{ width: `${(chairCount / tableSpec.capacity) * 100}%` }}
                                />
                              </div>
                              <span className="text-xs text-gray-500">{chairCount}/{tableSpec.capacity}</span>
                            </div>
                            {table.chairCount !== undefined && table.chairCount !== effectiveCap && (
                              <button
                                onClick={() => onUpdateTable(table.id, { chairCount: undefined })}
                          ...
                          {/* Seating-only row configuration summary */}
                          {isSeatingType && (
                            <div className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-800 space-y-1">
                              <div><span className="font-semibold">Rows:</span> {Math.max(1, tableSpec.seatingRowCount || 1)}</div>
                              <div><span className="font-semibold">Row Spacing:</span> {tableSpec.seatingRowSpacing ?? 3} ft</div>
                              <div><span className="font-semibold">Total Chairs:</span> {chairCount * Math.max(1, tableSpec.seatingRowCount || 1)}</div>
                            </div>
                          )}
                          
                          <p className="text-xs text-gray-400">
                            Max: {tableSpec.capacity} chairs • Total area: ~{Math.ceil(tableSpec.width + 3)}' × {Math.ceil(tableSpec.height + 3)}'
                          </p>
                        </div>
                      </div>

                      {/* Chair Layout for Rectangular Tables */}
                      {tableSpec.shape === 'rectangle' && !isSeatingType && (
                        <div className="mt-4 pt-4 border-t border-gray-200">
                          ...
                          <div className="space-y-2">
                            <label className="flex items-start gap-3 p-2 rounded-lg hover:bg-gray-50 cursor-pointer border border-gray-200">
                              <input
                                ...
                                checked={(table.chairLayout || tableSpec.defaultChairLayout || 'all-sides') === 'long-sides-only'}
                                onChange={() => onUpdateTable(table.id, { chairLayout: 'long-sides-only' })}
                                className="mt-1 text-[#4A1942] focus:ring-[#4A1942]"
                              />
                              <div>
                                <div className="font-medium text-sm text-gray-800">Long Sides Only</div>
                                <div className="text-xs text-gray-500">Chairs on long sides only (e.g., 4+4 for 8 chairs)</div>
                            ...
                            <label className="flex items-start gap-3 p-2 rounded-lg hover:bg-gray-50 cursor-pointer border border-gray-200">
                              <input
                                ...
                                name="chairLayout"
                                value="all-sides"
                                checked={(table.chairLayout || tableSpec.defaultChairLayout || 'all-sides') === 'all-sides'}
                                onChange={() => onUpdateTable(table.id, { chairLayout: 'all-sides' })}
                                className="mt-1 text-[#4A1942] focus:ring-[#4A1942]"
                              />
                              <div>
                                <div className="font-medium text-sm text-gray-800">All Sides</div>
                                <div className="text-xs text-gray-500">Chairs on all 4 sides (e.g., 3+3+1+1 for 8 chairs)</div>
                                <div className="mt-1 flex items-center gap-1">
                                  <div className="w-12 h-8 bg-amber-100 border border-amber-300 rounded relative flex items-center justify-center text-xs">
                                    <span className="absolute -top-2 text-[8px]">○ ○</span>
                                    <span className="absolute -bottom-2 text-[8px]">○ ○</span>
                                    <span className="absolute -left-1.5 top-1/2 -translate-y-1/2 text-[8px] flex flex-col gap-0.5">○</span>
                                    <span className="absolute -right-1.5 top-1/2 -translate-y-1/2 text-[8px] flex flex-col gap-0.5">○</span>
                                  </div>
                                </div>
                              </div>
                            </label>
                            ...
                            <label className="flex items-start gap-3 p-2 rounded-lg hover:bg-gray-50 cursor-pointer border border-gray-200">
                              <input
                                ...
                                name="chairLayout"
                                value="head-table"
                                checked={(table.chairLayout || tableSpec.defaultChairLayout || 'all-sides') === 'head-table'}
                                onChange={() => onUpdateTable(table.id, { chairLayout: 'head-table' })}
                                className="mt-1 text-[#4A1942] focus:ring-[#4A1942]"
                              />
                              <div>
                                <div className="font-medium text-sm text-gray-800">Head Table</div>
                                <div className="text-xs text-gray-500">Chairs on one long side only (facing guests)</div>
                                <div className="mt-1 flex items-center gap-1">
                                  <div className="w-12 h-6 bg-amber-100 border border-amber-300 rounded relative flex items-center justify-center text-xs">
                                    <span className="absolute -bottom-2 text-[8px]">○ ○ ○ ○</span>
                                  </div>
                          ...
                          {table.chairLayout && table.chairLayout !== (tableSpec.defaultChairLayout || 'all-sides') && (
                            <button
                              onClick={() => onUpdateTable(table.id, { chairLayout: undefined })}
