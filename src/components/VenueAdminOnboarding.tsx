...
import { acceptVenueAdminInvite } from '../services/platform/platformAdminService';
...
export default function VenueAdminOnboarding({ token }: VenueAdminOnboardingProps) {
  ...
  const handleExistingAccount = async () => {
    ...
    try {
      ...
      setState('success');
      setMessage(`You are now the managed administrator for ${result.organizationName}. Reloading your venue workspace…`);
      window.setTimeout(() => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (!token) {
      setState('error');
      setMessage('This venue administrator link is missing its invitation token.');
      return;
    }
    ...
    try {
      await signUpVenueAdminWithInvite({
      ...
      setState('success');
      setMessage('Your venue administrator account is ready. Opening the venue workspace…');
      window.setTimeout(() => {
        window.location.hash = '#/venue';
        window.location.reload();
      }, 900);
        ...
        <div className="w-full rounded-2xl border border-gray-200 bg-white p-6 shadow-xl sm:p-8">
          <div className="text-center">
            ...
            <p className="mt-2 text-sm leading-relaxed text-gray-600">
              Create the managed administrator account for this venue. This account will be able to configure the venue and invite its internal team.
            </p>
          </div>

          {!token ? (
            <div className="mt-6 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">This setup link is missing its token or is not valid.</div>
          ) : user ? (
            <div className="mt-6 space-y-4">
              <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900">
                You are signed in as <strong>{user.email || user.name}</strong>. Use this account to claim the venue administrator invitation.
              </div>
              <button
                type="button"
              ...
              >
                {state === 'saving' ? 'Claiming venue…' : 'Claim Venue Administrator Access'}
              </button>
            </div>
          ) : (
            <form onSubmit={(event) => void handleSubmit(event)} className="mt-6 space-y-3">
              ...
              <div>
                <label htmlFor="venue-admin-email" className="mb-1 block text-xs font-semibold text-gray-700">Email address</label>
                <input id="venue-admin-email" type="email" value={form.email} onChange={(event) => setForm((current) => ({ ...current, email: event.target.value }))} className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm" autoComplete="email" />
                <p className="mt-1 text-[11px] text-gray-500">Use the email address that received this invitation.</p>
              </div>
              <div>
                <label htmlFor="venue-admin-password" className="mb-1 block text-xs font-semibold text-gray-700">Password</label>
              ...
              <div>
                <label htmlFor="venue-admin-confirm-password" className="mb-1 block text-xs font-semibold text-gray-700">Confirm password</label>
              ...
              <button type="submit" disabled={state === 'saving'} className="w-full rounded-lg px-4 py-3 text-sm font-bold text-white disabled:opacity-60" style={{ backgroundColor: config.primaryColor || '#4A1942' }}>
                {state === 'saving' ? 'Creating account…' : 'Create Venue Administrator Account'}
              </button>
            </form>
          )}

          {message && (
            <div className={`mt-4 rounded-xl border p-3 text-sm ${state === 'error' ? 'border-red-200 bg-red-50 text-red-700' : state === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-gray-200 bg-gray-50 text-gray-700'}`} role={state === 'error' ? 'alert' : 'status'}>
              {message}
            </div>
          )}

          {state !== 'success' && (
            <button type="button" onClick={() => { window.location.hash = ''; }} className="mt-6 w-full text-center text-xs text-gray-500 hover:underline">Return to sign in</button>
          )}
        </div>
      </div>
    </div>
