...
import { mapBounds, markerColor } from '../utils/platformMapGeometry';
...
export default function PlatformVenueMap({ organizations }: PlatformVenueMapProps) {
  ...
  const bounds = useMemo(
    () => mapBounds(located.map((organization) => ({ latitude: Number(organization.latitude), longitude: Number(organization.longitude) }))),
    [located],
  );
  const locatedKey = located.map((organization) => `${organization.id}:${organization.latitude}:${organization.longitude}`).join('|');
  ...
  useEffect(() => {
    ...
    void import('leaflet').then((mod) => {
      ...
      const L = mod.default;
      const leafletMap = L.map(mapNode.current, { scrollWheelZoom: true }).setView(
        located.length ? [Number(located[0].latitude), Number(located[0].longitude)] : [39.8, -98.5],
        located.length ? 5 : 4,
      );
      ...
      const group: ReturnType<typeof L.latLngBounds> | null = located.length
        ? L.latLngBounds(located.map((organization) => [Number(organization.latitude), Number(organization.longitude)] as [number, number]))
        : null;
      located.forEach((organization) => {
        const marker = L.circleMarker([Number(organization.latitude), Number(organization.longitude)], {
          radius: selectedId === organization.id ? 10 : 7,
          color: '#ffffff',
          weight: 2,
          fillColor: markerColor(organization.status),
          fillOpacity: 0.95,
        }).addTo(leafletMap);
        marker.bindPopup(`<strong>${organization.name}</strong><br/>${[organization.city, organization.stateRegion].filter(Boolean).join(', ')}`);
        marker.on('click', () => setSelectedId(organization.id));
      });
      if (group) leafletMap.fitBounds(group.pad(0.2));
      map = leafletMap;
    }).catch((error) => {
      setTileError(error instanceof Error ? error.message : 'Could not start the map.');
    });
    return () => {
      cancelled = true;
      map?.remove();
    ...
    <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm" aria-label="Platform venue map">
      ...
      {view !== 'regions' && (
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-slate-100">
          {useTiles ? (
            <div ref={mapNode} className="h-[360px] w-full" aria-label="Geoapify map of venue organizations" />
          ) : (
