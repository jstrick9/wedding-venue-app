...
- **Supabase mode (`VITE_BACKEND_PROVIDER=supabase`):** an optional multi-tenant
  ...
  - Migrations live in `supabase/migrations/` (`0001`–`0022`). Apply them in order
    ...
    and a production Auth Site URL on the branded application origin. Production recovery uses one server-controlled origin and verified sender for every tenant; their non-confidential production defaults are versioned with the Function, while `PUBLIC_APP_URL` and `PASSWORD_RESET_FROM_EMAIL` remain environment overrides (`PASSWORD_RESET_APP_URL` is a legacy URL fallback). Browser-supplied non-local origins and sender values are rejected.
